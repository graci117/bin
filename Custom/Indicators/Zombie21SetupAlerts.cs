...
using NinjaTrader.NinjaScript.Indicators.Z21SA;
#endregion

namespace NinjaTrader.NinjaScript.Indicators
{

    public class Zombie21SetupAlerts : Indicator
    {

        private const string SystemVersion = "v1.028";
        private const string SystemName = "Zombie21SetupAlerts";
        ...
        private Brush barColorZombieBuy = Brushes.DimGray;
        private Brush barColorHalfLifeBuy = Brushes.DimGray;
        private Brush barColorZombieSell = Brushes.DimGray;
        private Brush barColorHalfLifeSell = Brushes.DimGray;
...
namespace NinjaTrader.NinjaScript.Indicators.Z21SA
{
...
namespace NinjaTrader.NinjaScript.Indicators
{
	...
	{
		private Zombie21SetupAlerts[] cacheZombie21SetupAlerts;
		public Zombie21SetupAlerts Zombie21SetupAlerts(string indicatorName, bool showZombieEntrySetups, bool showHalfLifeEntrySetups, bool showMomentumStack, int eMAFastPeriod, int eMAMiddlePeriod, int eMASlowPeriod, int signalBarPaddingTicks, Brush momentumStackBullishColor, Brush momentumStackBearishColor, Brush momentumStackLossColor, int momentumStackFontSize)
		{
			return Zombie21SetupAlerts(Input, indicatorName, showZombieEntrySetups, showHalfLifeEntrySetups, showMomentumStack, eMAFastPeriod, eMAMiddlePeriod, eMASlowPeriod, signalBarPaddingTicks, momentumStackBullishColor, momentumStackBearishColor, momentumStackLossColor, momentumStackFontSize);
		}

		public Zombie21SetupAlerts Zombie21SetupAlerts(ISeries<double> input, string indicatorName, bool showZombieEntrySetups, bool showHalfLifeEntrySetups, bool showMomentumStack, int eMAFastPeriod, int eMAMiddlePeriod, int eMASlowPeriod, int signalBarPaddingTicks, Brush momentumStackBullishColor, Brush momentumStackBearishColor, Brush momentumStackLossColor, int momentumStackFontSize)
		{
			if (cacheZombie21SetupAlerts != null)
				for (int idx = 0; idx < cacheZombie21SetupAlerts.Length; idx++)
					if (cacheZombie21SetupAlerts[idx] != null && cacheZombie21SetupAlerts[idx].IndicatorName == indicatorName && cacheZombie21SetupAlerts[idx].ShowZombieEntrySetups == showZombieEntrySetups && cacheZombie21SetupAlerts[idx].ShowHalfLifeEntrySetups == showHalfLifeEntrySetups && cacheZombie21SetupAlerts[idx].ShowMomentumStack == showMomentumStack && cacheZombie21SetupAlerts[idx].EMAFastPeriod == eMAFastPeriod && cacheZombie21SetupAlerts[idx].EMAMiddlePeriod == eMAMiddlePeriod && cacheZombie21SetupAlerts[idx].EMASlowPeriod == eMASlowPeriod && cacheZombie21SetupAlerts[idx].SignalBarPaddingTicks == signalBarPaddingTicks && cacheZombie21SetupAlerts[idx].MomentumStackBullishColor == momentumStackBullishColor && cacheZombie21SetupAlerts[idx].MomentumStackBearishColor == momentumStackBearishColor && cacheZombie21SetupAlerts[idx].MomentumStackLossColor == momentumStackLossColor && cacheZombie21SetupAlerts[idx].MomentumStackFontSize == momentumStackFontSize && cacheZombie21SetupAlerts[idx].EqualsInput(input))
						return cacheZombie21SetupAlerts[idx];
			return CacheIndicator<Zombie21SetupAlerts>(new Zombie21SetupAlerts(){ IndicatorName = indicatorName, ShowZombieEntrySetups = showZombieEntrySetups, ShowHalfLifeEntrySetups = showHalfLifeEntrySetups, ShowMomentumStack = showMomentumStack, EMAFastPeriod = eMAFastPeriod, EMAMiddlePeriod = eMAMiddlePeriod, EMASlowPeriod = eMASlowPeriod, SignalBarPaddingTicks = signalBarPaddingTicks, MomentumStackBullishColor = momentumStackBullishColor, MomentumStackBearishColor = momentumStackBearishColor, MomentumStackLossColor = momentumStackLossColor, MomentumStackFontSize = momentumStackFontSize }, input, ref cacheZombie21SetupAlerts);
		}
	}
}
...
namespace NinjaTrader.NinjaScript.MarketAnalyzerColumns
{
	...
	{
		public Indicators.Zombie21SetupAlerts Zombie21SetupAlerts(string indicatorName, bool showZombieEntrySetups, bool showHalfLifeEntrySetups, bool showMomentumStack, int eMAFastPeriod, int eMAMiddlePeriod, int eMASlowPeriod, int signalBarPaddingTicks, Brush momentumStackBullishColor, Brush momentumStackBearishColor, Brush momentumStackLossColor, int momentumStackFontSize)
		{
			return indicator.Zombie21SetupAlerts(Input, indicatorName, showZombieEntrySetups, showHalfLifeEntrySetups, showMomentumStack, eMAFastPeriod, eMAMiddlePeriod, eMASlowPeriod, signalBarPaddingTicks, momentumStackBullishColor, momentumStackBearishColor, momentumStackLossColor, momentumStackFontSize);
		}

		public Indicators.Zombie21SetupAlerts Zombie21SetupAlerts(ISeries<double> input , string indicatorName, bool showZombieEntrySetups, bool showHalfLifeEntrySetups, bool showMomentumStack, int eMAFastPeriod, int eMAMiddlePeriod, int eMASlowPeriod, int signalBarPaddingTicks, Brush momentumStackBullishColor, Brush momentumStackBearishColor, Brush momentumStackLossColor, int momentumStackFontSize)
		{
			return indicator.Zombie21SetupAlerts(input, indicatorName, showZombieEntrySetups, showHalfLifeEntrySetups, showMomentumStack, eMAFastPeriod, eMAMiddlePeriod, eMASlowPeriod, signalBarPaddingTicks, momentumStackBullishColor, momentumStackBearishColor, momentumStackLossColor, momentumStackFontSize);
		}
	}
