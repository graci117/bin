...
{
	public class HFT_RSI_SCALPER : Indicator
	{
		...
		protected override void OnStateChange()
		{
			...
			if (State != (State)2)
			{
				if (State == (State)4)
				{
					Draw.TextFixed(this, "hftName", "HFT RSI SCALPER", TextPosition.TopRight, Brushes.DodgerBlue, title, Brushes.Black,Brushes.Black,50);
		...
		protected override void OnBarUpdate()
		{
			try
			{
				...
				{	
					...
					{
						if(CrossAbove(RSI(14,3),30,2) && RSI(2,3)[0] >= 70)
						{
							Draw.Line(this, "Long Entry Line" + CurrentBar, false, 1, GetCurrentAsk(), -1, GetCurrentAsk(), Brushes.DeepSkyBlue, 0, 3);
							Draw.Text(this, "NQLong1" + CurrentBar, false, "▲", 0, Low[0] - (10.0 * TickSize), 0, Brushes.DodgerBlue, new SimpleFont("Arial", 20), TextAlignment.Center, Brushes.Transparent, Brushes.Transparent, 0);
							Draw.Line(this, "15 Target Line" + CurrentBar, false, 1, GetCurrentAsk() + tLine * TickSize, -1, GetCurrentAsk() + tLine * TickSize, Brushes.LimeGreen, 0, 2);
					...
					if(Close[1]<Open[1] && Close[0]<Open[0])
					{
						if(CrossBelow(RSI(14,3),70,2) && RSI(2,3)[0] <= 30)
						{
							Draw.Line(this, "Short Entry Line" + CurrentBar, false, 1, GetCurrentBid(), -1, GetCurrentBid(), Brushes.Magenta, 0, 3);
							Draw.Text(this, "NQShort1" + CurrentBar, false, "▼", 0, High[0] + (10.0 * TickSize), 0, Brushes.DodgerBlue, new SimpleFont("Arial", 20), TextAlignment.Center, Brushes.Transparent, Brushes.Transparent, 0);
							Draw.Line(this, "15 Target Line" + CurrentBar, false, 1, GetCurrentBid() - tLine * TickSize, -1, GetCurrentBid() - tLine * TickSize, Brushes.LimeGreen, 0, 2);
		...
		private FrameworkElement CreateControl()
		{
			try
			{
				...
				{
					Content = string.Format("HFT RSI SCALPER", 0),
					Height = 20.0,
				...
				else
				{
					try
					{
						...
						format2 = NinjaTrader.NinjaScript.Indicators.HFT_RSI_SCALPER.CensorString(this.accountSelector.SelectedAccount.DisplayName);
					}
					catch (Exception)
					{
						format2 = "Select Account";
		...
		private void ACCOUNTSelection_Changed(object sender, EventArgs e)
		{
			...
			if (this.accountSelector != null)
			{
				...
				this.accountSelected.Content = NinjaTrader.NinjaScript.Indicators.HFT_RSI_SCALPER.CensorString(this.accountSelector.SelectedAccount.DisplayName);
			}
		}
...
namespace NinjaTrader.NinjaScript.Indicators
{
	...
	{
		private HFT_RSI_SCALPER[] cacheHFT_RSI_SCALPER;
		public HFT_RSI_SCALPER HFT_RSI_SCALPER(int realizedProfit, int realizedLoss, int startTime, int endTime, int econNumber1, int econNumber2, int openTime, int tLine)
		{
			return HFT_RSI_SCALPER(Input, realizedProfit, realizedLoss, startTime, endTime, econNumber1, econNumber2, openTime, tLine);
		}

		public HFT_RSI_SCALPER HFT_RSI_SCALPER(ISeries<double> input, int realizedProfit, int realizedLoss, int startTime, int endTime, int econNumber1, int econNumber2, int openTime, int tLine)
		{
			if (cacheHFT_RSI_SCALPER != null)
				for (int idx = 0; idx < cacheHFT_RSI_SCALPER.Length; idx++)
					if (cacheHFT_RSI_SCALPER[idx] != null && cacheHFT_RSI_SCALPER[idx].realizedProfit == realizedProfit && cacheHFT_RSI_SCALPER[idx].realizedLoss == realizedLoss && cacheHFT_RSI_SCALPER[idx].startTime == startTime && cacheHFT_RSI_SCALPER[idx].endTime == endTime && cacheHFT_RSI_SCALPER[idx].econNumber1 == econNumber1 && cacheHFT_RSI_SCALPER[idx].econNumber2 == econNumber2 && cacheHFT_RSI_SCALPER[idx].openTime == openTime && cacheHFT_RSI_SCALPER[idx].tLine == tLine && cacheHFT_RSI_SCALPER[idx].EqualsInput(input))
						return cacheHFT_RSI_SCALPER[idx];
			return CacheIndicator<HFT_RSI_SCALPER>(new HFT_RSI_SCALPER(){ realizedProfit = realizedProfit, realizedLoss = realizedLoss, startTime = startTime, endTime = endTime, econNumber1 = econNumber1, econNumber2 = econNumber2, openTime = openTime, tLine = tLine }, input, ref cacheHFT_RSI_SCALPER);
		}
	}
}
...
namespace NinjaTrader.NinjaScript.MarketAnalyzerColumns
{
	...
	{
		public Indicators.HFT_RSI_SCALPER HFT_RSI_SCALPER(int realizedProfit, int realizedLoss, int startTime, int endTime, int econNumber1, int econNumber2, int openTime, int tLine)
		{
			return indicator.HFT_RSI_SCALPER(Input, realizedProfit, realizedLoss, startTime, endTime, econNumber1, econNumber2, openTime, tLine);
		}

		public Indicators.HFT_RSI_SCALPER HFT_RSI_SCALPER(ISeries<double> input , int realizedProfit, int realizedLoss, int startTime, int endTime, int econNumber1, int econNumber2, int openTime, int tLine)
		{
			return indicator.HFT_RSI_SCALPER(input, realizedProfit, realizedLoss, startTime, endTime, econNumber1, econNumber2, openTime, tLine);
		}
	}
