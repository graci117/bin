...
namespace NinjaTrader.NinjaScript.Indicators
{
    ...
    public class Zombie21Candles : Indicator
    {

        private const string SystemVersion = "v1.028";
        private const string SystemName = "Zombie21Candles";
        ...
        private Brush lastCandleColor = null;
        private Brush lastCandleOutlineColor = null;
        ...
        protected override void OnStateChange()
        {
            ...
            else if (State == State.DataLoaded)
            {
                ...
                max = MAX(High, PowerLinePeriod);
                min = MIN(Low, PowerLinePeriod);

                lastCandleColor = barColorCondition4;
                lastCandleOutlineColor = candleOutlineCondition4;
            }
        }
        ...
        protected override void OnBarUpdate()
        {
            ...
            double powerLineMeanPrice = (max0 + min0) / 2;
            ...
            bool rocketBullishPreviousCandle = Close[1] >= Open[2];

            bool rocketBullishCandle = (High[0] >= High[1] || Close[0] >= Open[1]) && (Low[0] >= Low[1] || (Close[0] >= Open[1]));
            bool rocketNeutralCandle = High[0] <= High[1] && Low[0] >= Low[1];

            bool powerLineBullishCandle = Close[0] >= powerLineMeanPrice;

            if (bullishTrend && !powerLineBullishCandle)
            {
                if (rocketBullishCandle && !rocketNeutralCandle)
                {
                    if (colorBars)
                    {
                    ...
                    lastCandleColor = barColorCondition5;
                    lastCandleOutlineColor = candleOutlineCondition5;
                }
                else if (!rocketBullishCandle && !rocketNeutralCandle)
                {
                    if (colorBars)
                    {
                    ...
                    lastCandleColor = barColorCondition6;
                    lastCandleOutlineColor = candleOutlineCondition6;
                }
                else
                {
                    BarBrush = lastCandleColor;
                    CandleOutlineBrush = lastCandleOutlineColor;
                }
            }
            else if (!bullishTrend && powerLineBullishCandle)
            {
                if (rocketBullishCandle && !rocketNeutralCandle)
                {
                    if (colorBars)
                    ...
                    lastCandleColor = barColorCondition1;
                    lastCandleOutlineColor = candleOutlineCondition1;
                }
                else if (!rocketBullishCandle && !rocketNeutralCandle)
                {
                    if (colorBars)
                    ...
                    lastCandleColor = barColorCondition2;
                    lastCandleOutlineColor = candleOutlineCondition2;
                }
                else
                {
                    BarBrush = lastCandleColor;
...
namespace NinjaTrader.NinjaScript.Indicators
{
	...
	{
		private Zombie21Candles[] cacheZombie21Candles;
		public Zombie21Candles Zombie21Candles(string indicatorName, int eMAFastPeriod, int eMAMiddlePeriod, int eMASlowPeriod, int powerLinePeriod)
		{
			return Zombie21Candles(Input, indicatorName, eMAFastPeriod, eMAMiddlePeriod, eMASlowPeriod, powerLinePeriod);
		}

		public Zombie21Candles Zombie21Candles(ISeries<double> input, string indicatorName, int eMAFastPeriod, int eMAMiddlePeriod, int eMASlowPeriod, int powerLinePeriod)
		{
			if (cacheZombie21Candles != null)
				for (int idx = 0; idx < cacheZombie21Candles.Length; idx++)
					if (cacheZombie21Candles[idx] != null && cacheZombie21Candles[idx].IndicatorName == indicatorName && cacheZombie21Candles[idx].EMAFastPeriod == eMAFastPeriod && cacheZombie21Candles[idx].EMAMiddlePeriod == eMAMiddlePeriod && cacheZombie21Candles[idx].EMASlowPeriod == eMASlowPeriod && cacheZombie21Candles[idx].PowerLinePeriod == powerLinePeriod && cacheZombie21Candles[idx].EqualsInput(input))
						return cacheZombie21Candles[idx];
			return CacheIndicator<Zombie21Candles>(new Zombie21Candles(){ IndicatorName = indicatorName, EMAFastPeriod = eMAFastPeriod, EMAMiddlePeriod = eMAMiddlePeriod, EMASlowPeriod = eMASlowPeriod, PowerLinePeriod = powerLinePeriod }, input, ref cacheZombie21Candles);
		}
	}
}
...
namespace NinjaTrader.NinjaScript.MarketAnalyzerColumns
{
	...
	{
		public Indicators.Zombie21Candles Zombie21Candles(string indicatorName, int eMAFastPeriod, int eMAMiddlePeriod, int eMASlowPeriod, int powerLinePeriod)
		{
			return indicator.Zombie21Candles(Input, indicatorName, eMAFastPeriod, eMAMiddlePeriod, eMASlowPeriod, powerLinePeriod);
		}

		public Indicators.Zombie21Candles Zombie21Candles(ISeries<double> input , string indicatorName, int eMAFastPeriod, int eMAMiddlePeriod, int eMASlowPeriod, int powerLinePeriod)
		{
			return indicator.Zombie21Candles(input, indicatorName, eMAFastPeriod, eMAMiddlePeriod, eMASlowPeriod, powerLinePeriod);
		}
	}
