    ...
    public class BltTriggerLines : Indicator
    {
        ...
        protected override void OnBarUpdate()
        {
            ...
            if (CrossAbove(Trigger, Average, 1))
            {
                if (DrawArrows)
                {
                    Draw.ArrowUp(this, "ArrowUp"+CurrentBar, false, 0, Average[0]-ArrowTickOffset, ArrowUpColor);
                    ArrowDrawn = true;
                }
            ...
            else if (CrossBelow(Trigger, Average, 1))
            {
                if (DrawArrows)
                {
                    Draw.ArrowDown(this, "ArrowDown"+CurrentBar, false, 0, Average[0]+ArrowTickOffset, ArrowDownColor);
                    ArrowDrawn = true;
                }
                if (SoundAlert && !SoundPlayed)
                {
            ...
            else
            {
                if (ArrowDrawn)
                {
                    RemoveDrawObject("ArrowUp"+CurrentBar);
					RemoveDrawObject("ArrowDown"+CurrentBar);
                    ArrowDrawn = false;
                }
            }
