...
namespace NinjaTrader.NinjaScript.BarsTypes
{
	public class MinuteBarsType : BarsType
	{
		...
		public override double GetPercentComplete(Bars bars, DateTime now)
			=> now <= bars.LastBarTime ? 1.0 - bars.LastBarTime.Subtract(now).TotalMinutes / bars.BarsPeriod.Value : 1;
		...
		protected override void OnStateChange()
		{
			...
			{
				Name = string.Format(Core.Globals.GeneralOptions.CurrentCulture, Custom.Resource.DataBarsTypeMinute, BarsPeriod.Value, BarsPeriod.MarketDataType != MarketDataType.Last ? $" - {Core.Globals.ToLocalizedObject(BarsPeriod.MarketDataType, Core.Globals.GeneralOptions.CurrentUICulture)}" : string.Empty);
